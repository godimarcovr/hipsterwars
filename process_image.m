...
% x_patch_halfdim = patch_halfdim;
% y_patch_halfdim = patch_halfdim;
...
hold on
plot(round(coordinates((2*ind)-1, :)), round(coordinates((2*ind), :)), 'rx');
hold off
subplot(1,3,2);
imagesc(item_scores_map)
title([names{style_ind} ' ' posneg])
colorbar
subplot(1,3,3);
imagesc(part_scores_map)
colorbar
